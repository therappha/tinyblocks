 ...
 *
 * required = false: confirmed still broken (diagnostic assertions show the piston circuit itself
 * — power, extend, push — all work correctly; only applyChanges' cross-boundary "new piece"
 * handling fails to land the piece in a newly-created neighbor). Kept enabled and reported rather
 * than deleted so the suite documents the exact live failure and flips green automatically
 * whenever #34 actually gets fixed, instead of silently regressing back to "no coverage."
 */
